...
counties <- read_csv("https://raw.githubusercontent.com/nytimes/covid-19-data/
...
ratio_states <- mutate(states, deaths /cases)
state_highest_ratio <- ratio_states %>%
  filter(date == max(date)) %>%
...
counties <- unite(counties, location, county, state, sep = ", ", remove = F)
...
# How many people died on the date when the most new deaths occured? `most_deaths`
...
prop_no_deaths <- counties%>%
  filter(date == max(date))%>% 
  group_by(state)%>% 
  summarize(total_county = n(), total_zero_county = sum(deaths == 0))%>%
  mutate(prop = total_zero_county/total_county)%>%
  select(state, prop)
...
#Question1:
#Which state has had the highest number of cases *as the most recent date*?
# `state_highest_cases_recent`
state_highest_cases_recent <- states %>%
  filter(date == max(date))%>%
  filter(cases == max(cases)) %>%
  pull(state)

# Q2: What is the name of the location (county, state) with the highest number
# of cases? `location_most_cases`
location_most_cases <- counties%>%
  filter(cases == max(cases, na.rm = TRUE))%>%
  pull(location)

# Q3: What is the *highest absolute difference* between the sum of the county 
# cases and the reported state cases. `state_hd`.
state_hd <- has_discrepancy%>%
  mutate(abs_diff = abs(county_totals - cases))%>%
  filter(abs_diff == max(abs_diff))%>%
  pull(abs_diff)
